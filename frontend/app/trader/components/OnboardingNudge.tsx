 ...
 *   - Green banner (live) → localStorage. Survives sign-out and
 *     sign-back-in — it's a one-time "you made it" celebration.
 *     Clearing the skip flag on logout (auth.tsx) does NOT touch
 *     this key.
 *
...
const PERMANENT_DISMISS_KEY = "onboarding_nudge_dismissed:live";
...
function writeDismissed(kind: BannerKind): void {
...
function readAllDismissed(): Set<BannerKind> {
  const s = new Set<BannerKind>();
  if (readDismissed("pick"))   s.add("pick");
  if (readDismissed("deploy")) s.add("deploy");
  if (readDismissed("live"))   s.add("live");
  return s;
}
