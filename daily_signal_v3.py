...
daily_signal_v3.py
==================
Canonical daily-signal generator — basket source: peek (amber-direct).

Replaces v2's Binance FAPI REST snapshot with peek's Amberdata-direct
basket logic. Everything else (Tail Guardrail, Dispersion, BloFin gate,
conviction roi_x, DB writes, CSV writes, per-strategy decisions) is
preserved verbatim from v2.

Why the swap (cutover 2026-04-28)
---------------------------------
Empirical 21-day comparison (2026-04-07 → 2026-04-27) showed peek's
basket outperformed v2's basket by +13.4pp cumulative over 18 overlapping
days (avg +0.36% vs -0.38% per day, peek won 10/18 days). The audit-
canonical basket — which peek reproduces byte-for-byte on past dates
(verified 6/6 month-start matches) — produces materially better deploys
than v2's Binance-REST snapshot.

Mechanism: peek reads from Amberdata via the same path the audit uses,
with 1-minute OI bars and exact 06:00 close reference. v2's path used
5-minute OI bars and an off-canonical snapshot timing, which
disadvantaged v2 at the rank-20 boundary.

v2 is preserved at archive/daily_signal_v2_archived_2026-04-28.py for
fallback if needed (rollback path: switch the cron back to v2).

Pipeline (this file):
  1. Query BloFin instrument list (exchange-availability gate).
  2. Snapshot at 06:00 UTC via peek's amber-direct fetcher: for each
     Binance USDT-perp, fetch 1-minute OHLCV close + per-minute open
     interest from Amberdata, anchor at 00:00 UTC, rank top-20 by
     pct_change for each metric. Basket = intersection of the two
     top-20 sets, normalized.
  3. BloFin filter: drop basket symbols not on BloFin.
  4. Tail Guardrail: BTC prev-day return < -4% OR 5d rvol > 1.4x 60d
     baseline → sit flat.
  5. Write to live_deploys_signal.csv (same path the trader reads).
  6. Write to user_mgmt.daily_signals + daily_signal_items DB, one row
     per published+active strategy_version_id.

NOTE on data source: v3 reads Amberdata directly via peek. The nightly
indexer cron (01:00 UTC) populates market.leaderboards for day D-1, not
D, so peek-db doesn't have today's data at 06:00 UTC — peek's amber-
direct path bypasses this. Universe is fetched fresh per run from
Amberdata's open-interest/information endpoint (matches metl.py's
universe).

Usage (cron entry):
  0 6 * * *  /root/benji/pipeline/.venv/bin/python /root/benji/daily_signal_v3.py \\
...
def compute_canonical_basket(ref_date):
    """Compute the audit-canonical basket via peek's amber-direct fetcher.

    Reads 1-minute OHLCV close + open-interest bars from Amberdata for
    every Binance USDT-perp in the universe. Anchors at 00:00 UTC,
    snapshots at 06:00 UTC, ranks top-20 by pct_change for each metric,
    intersects the two top-20 sets after normalize_symbol.

    This replaces v2's Binance FAPI REST snapshot. The audit pipeline
    uses the same Amberdata data via market.leaderboards; peek
    reproduces the audit's basket byte-for-byte on past dates (6/6
    month-start matches verified 2026-04-28). Live trader's basket now
    matches the audit's canonical basket on the same date — closing
    the audit-vs-live divergence (cumulative +13.4pp v2 underperform
    over 21 days).

    Returns (basket_list, price_top_rows, oi_top_rows).
        basket_list     = sorted list of normalized base symbols
        price_top_rows  = [(base, pct_change), ...]  (length ≤ 20)
        oi_top_rows     = [(base, pct_change), ...]  (length ≤ 20)
    """
    # Lazy import — keeps the module loadable even if peek's deps are
    # unavailable for static-check purposes.
    sys.path.insert(0, str(_SCRIPT_DIR))  # so `from pipeline import peek` resolves
    from pipeline import peek as _peek  # noqa: E402

    snap_dt = _dt.datetime.combine(
        ref_date, _dt.time(DEPLOYMENT_START_HOUR, 0, 0),
        tzinfo=_dt.timezone.utc,
    )
    log.info(f"Computing canonical basket (peek amber-direct) "
             f"snapshot_target={snap_dt.isoformat()}")

    t0 = time.time()
    # peek's fetch_amber_with_cache reads from /mnt/quant-data/peek_cache,
    # populates incrementally per symbol since 00:00 UTC, returns a
    # DataFrame of (symbol, ts_min, value) bars. For peek_date == today,
    # the per-symbol-delta cache is honored. force=False so iterative cron
    # runs benefit from any earlier same-day fills (e.g. peek runs by an
    # operator before 06:00).
    price_df = _peek.fetch_amber_with_cache(
        ref_date, "price", snap_dt, force=False)
    oi_df = _peek.fetch_amber_with_cache(
        ref_date, "open_interest", snap_dt, force=False)
    log.info(f"  Fetched price={len(price_df):,} rows, "
             f"oi={len(oi_df):,} rows in {time.time() - t0:.1f}s")

    # peek's compute_ranks_from_cache:
    #   - filters to [00:00, snap_ts_min] inclusive
    #   - per symbol: anchor = first bar's value, snap = last bar's value
    #   - drops symbols where anchor <= 0 (matches indexer's hygiene)
    #   - normalizes binance_id -> base via overlap_analysis.normalize_symbol
    #   - returns [(rank, base, pct_change_decimal), ...] sorted by rank ASC
    price_ranks_full = _peek.compute_ranks_from_cache(price_df, ref_date, snap_dt)
    oi_ranks_full = _peek.compute_ranks_from_cache(oi_df, ref_date, snap_dt)

    if not price_ranks_full:
        log.error("No price data returned from Amberdata — aborting basket.")
        return [], [], []
    if not oi_ranks_full:
        log.warning("No OI data returned from Amberdata — basket will be empty.")

    # Convert peek's (rank, base, pct) → v2's (id, pct_change) shape so
    # downstream code (logging only — not used for matching) stays happy.
    price_ranks = [(b, pct) for _, b, pct in price_ranks_full]
    oi_ranks = [(b, pct) for _, b, pct in oi_ranks_full]

    price_bases = {b for _, b, _ in price_ranks_full}
    oi_bases = {b for _, b, _ in oi_ranks_full}
    basket = sorted(price_bases & oi_bases)

    log.info(f"  Price top-{len(price_ranks)}: {[b for b, _ in price_ranks]}")
    log.info(f"  OI top-{len(oi_ranks)}:       {[b for b, _ in oi_ranks]}")
