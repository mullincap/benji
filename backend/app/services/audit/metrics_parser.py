"""Scrape metrics from a completed audit pipeline's audit_output.txt.

Pure function with no orchestration side effects — takes a Path, returns a
dict. Extracted from app.workers.pipeline_worker during Track 3 Item 4 so all
three callers (Simulator Celery task, nightly CLI, GET /api/jobs defensive
re-parse) import from a single location.
...
def parse_metrics(audit_output_path: Path) -> dict:
