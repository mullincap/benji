...
def write_to_db(date_str: str, filter_name: str, overlap_pool: list,
    ...
    Post-Session-E: writes one row per published+active strategy_version_id
    instead of a single hardcoded STRATEGY_VERSION_ID. Alpha Med/High lev
    strategies are derivative of v1.0's filter — they share the same
    overlap_pool / sit_flat / filter_name but need their own signal rows so
    allocation-keyed queries (manager overview, trader subprocesses) can find
    signals matching their strategy_version_id.

    A DB failure is logged but never fatal.
    """
    try:
            ...
            if secrets_path.exists():
                for line in secrets_path.read_text().splitlines():
                    ...
                    if line.startswith("STRATEGY_VERSION_ID="):
                        ...
                        break
            if not version_ids:
                log.warning("No published+active strategy versions and no STRATEGY_VERSION_ID fallback — skipping DB write")
                conn.close()
                return
        ...
        for sv_id in version_ids:
            ...
            row = cur.fetchone()
            if row is None:
                log.info(f"DB write: signal already exists for {sv_id[:8]} on {date_str} — skipped")
                continue
        ...
        cur.close()
        conn.close()
        log.info(f"DB write: {rows_written}/{len(version_ids)} strategy versions, {len(overlap_pool)} symbols")
